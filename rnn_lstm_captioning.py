...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    cache['Wx'], cache['Wh'], cache['b'] = Wx, Wh, b
    cache['X'], cache['prev_h'], cache['next_h']= x, prev_h, next_h
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    dnext_h = dnext_h * (1-cache['next_h']**2)
    dx = dnext_h @ cache['Wx'].T
    dprev_h = dnext_h @ cache['Wh'].T
    dWx = cache['X'].T @ dnext_h
    dWh = cache['prev_h'].T @ dnext_h
    db = torch.sum(dnext_h, axis=0)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape
    h0n, h0h = h0.shape
    h = torch.zeros(N, T, h0h, device=x.device)
    cache = []
    prev_h = h0
...
def rnn_backward(dh, cache):
    ...
    dnext_h = torch.zeros(N,H,device=dh.device)
    dx = torch.zeros(N,T,D, device=dh.device)
    dWx = torch.zeros(D, H, device=dh.device)
    dWh = torch.zeros(H, H, device=dh.device)
    db = torch.zeros(H, device=dh.device)

    for t in reversed(range(T)):
        dnext_h += dh[:,t,:] # Accumulate the gradient from the loss at this timestep
        tdx, tdprev_h, tdWx, tdWh, tdb = rnn_step_backward(dnext_h, cache[t])
        dx[:,t,:] = tdx
        dWx += tdWx
        dWh += tdWh
        db += tdb
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = torch.nn.functional.cross_entropy(x.reshape(-1, x.shape[2]), y.reshape(-1), 
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if self.cell_type == "rnn":
            self.rnn_to_use = RNN(wordvec_dim, hidden_dim)
        if self.cell_type == "lstm":
            self.rnn_to_use = LSTM(wordvec_dim, hidden_dim)
        if self.cell_type == "attn":
            self.rnn_to_use = AttentionLSTM(wordvec_dim, hidden_dim)
            self.images_to_A = nn.Conv2d(input_dim, hidden_dim, kernel_size=1)


        self.image_encoder = ImageEncoder(verbose=False)
        self.images_to_h0 = nn.Linear(input_dim, hidden_dim)
        self.word_embedder = WordEmbedding(self.vocab_size, wordvec_dim)
    ...
    def forward(self, images, captions):
        ...
        N, C, im_H, im_W = images.shape
        images_features_origi = self.image_encoder(images) #Nx400x4x4
            
        pooled_features = nn.functional.adaptive_avg_pool2d(images_features_origi,(1,1)) #Nx400x1x1
        images_features = pooled_features.reshape(N, -1) #Nx400

        h0 = self.images_to_h0(images_features)
        embedded_words = self.word_embedder(captions_in)

        if self.cell_type == "attn":
            A = self.images_to_A(images_features_origi) #NxHx4x4
            hn = self.rnn_to_use.forward(embedded_words, A) #NxTxH
        else:
            hn = self.rnn_to_use.forward(embedded_words, h0) #NxTxH

        N, T, H = hn.shape
        scores = self.output_layer(hn.reshape(N*T,-1)).reshape(N,T,-1)
    ...
    def sample(self, images, max_length=15):
        ...
        images_features_origi = self.image_encoder(images) #Nx400x4x4
        pooled_features = nn.functional.adaptive_avg_pool2d(images_features_origi,(1,1))
        images_features = pooled_features.reshape(N, -1) #Nx400

        prev_h = self.images_to_h0(images_features)
        prev_c = torch.zeros_like(prev_h)

        if self.cell_type == "rnn":
            for i in range(1,max_length):
                prev_h = self.rnn_to_use.step_forward(self.word_embedder(captions[:,i-1]), prev_h)
                scores = self.output_layer(prev_h)
                next_w = torch.argmax(scores, dim=1)
                captions[:,i] = next_w

        if self.cell_type == "lstm":
            for i in range(1,max_length):
                prev_h, prev_c = self.rnn_to_use.step_forward(self.word_embedder(captions[:,i-1]), prev_h, prev_c)
                scores = self.output_layer(prev_h)
                next_w = torch.argmax(scores, dim=1)
                captions[:,i] = next_w
        
        if self.cell_type == "attn":
            A = self.images_to_A(images_features_origi)
            prev_h = A.mean(dim=[2,3])
            prev_c = prev_h.clone()
            for i in range(1,max_length):
                attn, attn_weights = dot_product_attention(prev_h, A)
                prev_h, prev_c = self.rnn_to_use.step_forward(self.word_embedder(captions[:,i-1]), prev_h, prev_c, attn)
                scores = self.output_layer(prev_h)
                next_w = torch.argmax(scores, dim=1)
                captions[:,i] = next_w
        ...
        ######################################################################
        if self.cell_type == "attn": 
            attn_weights_all = attn_weights_all[:,1:,:,:]
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        af = torch.sigmoid(a[:,:H])   # N x H
        ai = torch.sigmoid(a[:,H:2*H])  # N x H
        ag = torch.tanh(a[:,2*H:3*H])  # N x H
        ao  = torch.sigmoid(a[:,3*H:])  # N x H

        next_c = af * prev_c + (ai * ag)
        next_h = torch.tanh(next_c) * ao
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(T):
            next_h, next_c = self.step_forward(x[:,t,:], h0, c0)
            hn[:,t,:] = next_h
            h0, c0 = next_h, next_c
...
def dot_product_attention(prev_h, A):
    ...
    h_ = prev_h.view(N,H,1,1)
    attn_weights = (h_*A).sum(dim=1).div(math.sqrt(H))
    attn_weights = torch.softmax(attn_weights.view(N,-1), dim=1) # N x 16
    attn_weights = attn_weights.view(N,D_a,D_a)  # N x 4 x 4

    attn = (A * attn_weights.unsqueeze(1)).sum(dim=[2,3]) # N x H
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        af = torch.sigmoid(a[:,:H])   # forget gate N x H
        ai = torch.sigmoid(a[:,H:2*H])  # input gate N x H
        ag = torch.tanh(a[:,2*H:3*H])  # gate gate N x H
        ao  = torch.sigmoid(a[:,3*H:])  # output gate N x H
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        c0 = h0.clone()  # Initial cell state, of shape (N, H)
        ...
        for t in range(T):
            ...
            next_h, next_c = self.step_forward(x[:,t,:], h0, c0, attn)
            hn[:,t,:] = next_h
            h0, c0 = next_h, next_c
        ######################################################################
        #                           END OF YOUR CODE                         #
